...
1. 更新任何调用 `addSegmentations` 的地方，以根据需要包含 `suppressEvents` 参数。
...
1. 更新任何创建或操作 `SegmentationPublicInput` 对象的代码，以根据需要包含新的 `config` 属性。
2. 将特定的分割数据类型替换为通用的 `RepresentationData` 类型。

### 添加分割表示

#### 以视口为中心的方法

该 API 现在关注于视口而非工具组，提供了对分割表示的更精细控制。
...
3. 更新任何依赖于返回的分割表示 UID Promise 的代码。

#### RepresentationPublicInput 变更

`RepresentationPublicInput` 类型已被简化，且部分属性已被重命名或移除。
...
1. 移除 `options` 属性并将 `colorLUTOrIndex` 移至 `config` 对象。
2. 如果使用，移除 `segmentationRepresentationUID` 和 `polySeg` 属性，polySEG 默认为启用状态。
3. 更新 `colorLUTOrIndex` 类型以接受 `Types.ColorLUT` 数组而非单一值。

#### 新的表示特定函数

版本 2 引入了用于向视口添加特定类型分割表示的新函数。
...
1. 使用适当的表示特定函数替换通用的 `addSegmentationRepresentations` 调用。
2. 更新输入数组以匹配新的 `RepresentationPublicInput` 类型。
3. 从代码中移除任何类型特定的逻辑，因为这些逻辑现在由这些新函数处理。

#### 多视口函数

版本 2 引入了用于同时向多个视口添加分割表示的新函数。
...
1. 如果您之前向多个工具组添加表示，请重构您的代码以使用这些新的多视口函数。
2. 创建一个 `viewportInputMap` 对象，以视口 ID 为键，`RepresentationPublicInput` 数组为值。
3. 根据表示类型调用适当的多视口函数。
...
在 Cornerstone3D 2.x 版本中，我们对分割配置 API 进行了重大重构，以提供更灵活和统一的方法来管理不同表示形式（Labelmap、Contour、Surface）的分割样式。旧的用于获取和设置分割配置的 API 已被替换为使用描述符对象（specifier object）来针对特定的分割、视口和分段的新函数。
...
#### 获取和设置特定于 ToolGroup 的配置

特定于 ToolGroup 的配置已被移除，取而代之的是特定视口的样式。以下示例将为特定视口和特定分割设置样式。
...
在 Cornerstone3D 2.x 版本中，用于获取和设置特定分割表示配置的函数已被统一的样式管理 API 所取代。旧的函数：

`getSegmentationRepresentationSpecificConfig`  
`setSegmentationRepresentationSpecificConfig`

不再可用。相反，你应该使用带有描述符对象的 getStyle 和 setStyle 函数来针对特定的分割和表示。
...
- **统一样式管理**：新的 `getStyle` 和 `setStyle` 函数为在不同层面（全局、特定分割、特定视口和特定分段）管理分割样式提供了统一的方法。
- **描述符对象（specifier）**：`specifier` 对象允许你针对特定的视口、分割和分段。
  - `type` 是必需的
  - 如果提供了 `segmentationId`，样式将应用于所有视口中的特定分割表示
  - 如果提供了 `segmentationId` 和 `segmentIndex`，样式将应用于特定分割表示的特定分段
  - 如果提供了 `viewportId`，样式将应用于特定视口中的所有分割
  - 如果同时提供了 `viewportId`、`segmentationId` 和 `segmentIndex`，样式将应用于特定视口中特定分割的特定分段
- **样式层级结构**：有效样式由考虑全局样式、特定分割样式和特定视口样式的层级结构决定。
...
1. 在函数调用中将所有 `toolGroupId` 替换为 `viewportId`。
2. 更新 `getActiveSegmentationRepresentation` 和 `getActiveSegmentation` 调用以使用新的 `getActiveSegmentation` 函数。
3. 使用新的参数结构将 `setActiveSegmentationRepresentation` 调用替换为 `setActiveSegmentation`。
...
1. 将所有 `getActiveSegmentationRepresentation` 的调用替换为 `getActiveSegmentation`。
2. 更新任何依赖于 `ToolGroupSpecificRepresentation` 类型的代码，以使用 `Segmentation` 类型。
